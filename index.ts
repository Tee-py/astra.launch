...
const instance = new aws.ec2.Instance("validator-instance", {
    ...
    userData: pulumi.all([fetchNodeId()]).apply(([nodeId]) =>
        `#!/bin/bash
set -e
...
MNEMONIC=$(echo "$INIT_OUTPUT" | grep -oP '(?<=your mnemonic : ).*')
ADDRESS=$(echo "$INIT_OUTPUT" | grep -oP 'kii[a-zA-Z0-9]+')
...
async function cleanup(
    ...
    try {
        ...
        await ec2.waitFor("instanceTerminated", { InstanceIds: [instanceId] })
            .promise();
        ...
        try {
            const attachedPoliciesResponse = await iam.listAttachedRolePolicies(
                { RoleName: roleName },
            ).promise();
...
// Error handling
export const cleanupFunction = pulumi.all([
    instance.id,
    eip.id,
    vpc.id,
    securityGroup.id,
